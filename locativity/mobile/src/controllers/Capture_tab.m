...
- (void)update_gui
{
    ...
    NSString *uri = [NSString stringWithFormat:@"http://www.sumukh.me/gps.php?lat=%f&long=%f",location.coordinate.latitude,location.coordinate.longitude];
    LOG(@" URL : %@",uri);
    NSString* apiUrlStr = uri; // First create an NSString with the link that you need to query:
    
    NSURL* apiUrl = [NSURL URLWithString:apiUrlStr]; // Convert the String to an NSURL.
    
    NSString *apiResponse = [NSString stringWithContentsOfURL:apiUrl encoding:NSASCIIStringEncoding error:&error]; // Query the URL with the method stringWithContentsofURL and store the response in a new string. I’ve called this string apiResponse:
    LOG(@"%@",apiResponse);
